...
def get_step(target, target_library):
    ...
    try:
        target = target + "/"
        return cf.binding_divided_by_base(target, target_library)
...
def plot(data, target_library, temp, color):
    ...
    sns.lineplot(data=stabilities_df, x='step', y='nb of binded base', hue='temp', palette=[color])
    
    plt.show()

def main():
    if len(sys.argv) != 4:
        print("usage: python3 comparison_curv_fit_binding.py <type of l> <target>")
    ...
    for target in targets:
        ...
        if len(stabilities) != 9:
            continue
        print(f"nb of binding bases for {target}: {stabilities}")
        for step in steps:
  ...
  # 'husl' で6色を生成
    ...
    plt.savefig(f'/home/user/SA-EDS/fig/curv_{target_library}_{sys.argv[2]}_binding.png') 
